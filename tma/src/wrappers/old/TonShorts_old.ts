...
function dictValueParserAdd(): DictionaryValue<Add> {
 ...
 type TonShorts_init_args = {
    $$type: 'TonShorts_init_args';
    id: bigint;
}

function initTonShorts_init_args(src: TonShorts_init_args) {
    return (builder: Builder) => {
        let b_0 = builder;
...
const TonShorts_getters: ABIGetter[] = [
...
const TonShorts_receivers: ABIReceiver[] = [
    {"receiver":"internal","message":{"kind":"typed","type":"Add"}},
    {"receiver":"internal","message":{"kind":"typed","type":"Deploy"}},
]

export class TonShorts implements Contract {
    
    static async init(id: bigint) {
        return await TonShorts_init(id);
    }
    
    static async fromInit(id: bigint) {
        const init = await TonShorts_init(id);
        const address = contractAddress(0, init);
        return new TonShorts(address, init);
    }
    
    static fromAddress(address: Address) {
        return new TonShorts(address);
    }
    
    readonly address: Address; 
    readonly init?: { code: Cell, data: Cell };
    readonly abi: ContractABI = {
        types:  TonShorts_types,
        getters: TonShorts_getters,
        receivers: TonShorts_receivers,
        errors: TonShorts_errors,
    };
